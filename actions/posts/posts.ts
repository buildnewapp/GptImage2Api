...
/**
 * Normalizes a PostgreSQL array_agg result to a JavaScript string array.
 * In Cloudflare Workers + Hyperdrive, array_agg returns a PG literal like `{val1,val2}`
 * instead of a JS array. This handles both formats.
 */
...
export async function createPostAction({
  ...
  try {
    ...
    if (postData.status === 'published') {
      const localePrefix = postData.language === DEFAULT_LOCALE ? '' : '/' + postData.language
      revalidatePath(`${localePrefix}/${postType}`)
      revalidatePath(`${localePrefix}/${postType}/${postData.slug}`)
    }
...
export async function updatePostAction({
  ...
  try {
    ...
    const oldLocalePrefix = currentPost.language === DEFAULT_LOCALE ? '' : '/' + currentPost.language
    revalidatePath(`${oldLocalePrefix}/${finalPostType}`)
    revalidatePath(`${oldLocalePrefix}/${finalPostType}/${currentPost.slug}`)

    // Revalidate new paths if language/slug changed or if published
    if (postUpdateData.status === 'published' ||
      postUpdateData.language !== currentPost.language ||
      postUpdateData.slug !== currentPost.slug) {
      const newLocalePrefix = postUpdateData.language === DEFAULT_LOCALE ? '' : '/' + postUpdateData.language
      revalidatePath(`${newLocalePrefix}/${finalPostType}`)
      revalidatePath(`${newLocalePrefix}/${finalPostType}/${postUpdateData.slug}`)
    }
...
export async function deletePostAction({
  ...
  try {
    ...
    if (postDetails?.slug && postDetails?.language) {
      const localePrefix = postDetails.language === DEFAULT_LOCALE ? '' : '/' + postDetails.language
      revalidatePath(`${localePrefix}/${postDetails.postType}`)
      revalidatePath(`${localePrefix}/${postDetails.postType}/${postDetails.slug}`)
    }
...
export async function listPublishedPostsAction({
  ...
  try {
    ...
    const postsWithProcessedTags = (data || []).map((post) => {
      const tagNames = parsePostgresArray(post.tag_names).join(', ') || null
