...
export default function HeroPhotoWall({ images }: HeroPhotoWallProps) {
    ...
    return (
        <div className="absolute inset-0 overflow-hidden bg-[#050b14]">
            <style>
                {`
                    @keyframes hero-photo-wall-scroll-up {
                        from { transform: translate3d(0, -33.333%, 0); }
                        to { transform: translate3d(0, -66.666%, 0); }
                    }

                    @keyframes hero-photo-wall-scroll-down {
                        from { transform: translate3d(0, -66.666%, 0); }
                        to { transform: translate3d(0, -33.333%, 0); }
                    }

                    @media (prefers-reduced-motion: reduce) {
                        [data-hero-photo-wall-column] {
                ...
                {photoColumns.map((columnImages, columnIndex) => (
                    ...
                    >
                        <div
                            data-hero-photo-wall-column
                            style={{
                                animationDelay: `${-columnIndex * 2.4}s`,
                                animationDuration: `${24 + columnIndex * 3}s`,
                                animationIterationCount: "infinite",
                                animationName:
                                    columnIndex % 2 === 0
                                        ? "hero-photo-wall-scroll-up"
                                        : "hero-photo-wall-scroll-down",
