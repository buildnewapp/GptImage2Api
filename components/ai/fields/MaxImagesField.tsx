...
export function isMaxImagesFieldDescriptor(
  field: AiVideoStudioFieldDescriptor,
) {
  return field.key === "max_images";
}
...
function shouldReturnNumberAsString(schema: JsonSchema) {
  ...
  return (
    ...
    (enumOptions.length > 0 &&
      enumOptions.every((option) => typeof option === "string"))
  );
}
...
function getMaxImageOptions(schema: JsonSchema) {
  const enumNumbers = getEnumOptions(schema)
    .map((option) => Number(option))
    .filter((option) => Number.isFinite(option));
...
export default function MaxImagesField({
  field,
  inputId,
  label,
  value,
  ...
  const options = getMaxImageOptions(field.schema);
  ...
  const formatValue = (nextValue: number) => {
    ...
    return shouldReturnNumberAsString(field.schema)
      ? String(normalized)
      : normalized;
  };
  const labelElement = (
  ...
  return (
    <div
      data-ai-video-studio-max-images-field={field.key}
