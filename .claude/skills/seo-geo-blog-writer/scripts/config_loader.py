...
class ConfigLoader:
    ...
    def get_kie_config(self,
                       api_key: Optional[str] = None,
                       resolution: Optional[str] = None) -> Dict[str, Optional[str]]:
        """
        Get Kie GPT Image 2 configuration

        Args:
            api_key: CLI-provided Kie API key (highest priority)
            resolution: CLI-provided image resolution: 1K, 2K, or 4K

        Returns:
            Dict with keys: api_key, resolution
        """
        return {
            'api_key': self.get('image_generation', 'kie_api_key', 'KIE_API_KEY', api_key),
            'resolution': self.get('image_generation', 'kie_resolution', 'KIE_IMAGE_RESOLUTION', resolution, '1K')
        }
    ...
    def get_image_generation_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'provider': self.get('image_generation', 'provider', 'IMAGE_GENERATION_PROVIDER', None, 'auto'),
            'max_images_per_post': self.get('image_generation', 'max_images_per_post', None, None, 5),
            'body_images_per_post': self.get('image_generation', 'body_images_per_post', None, None, 4),
