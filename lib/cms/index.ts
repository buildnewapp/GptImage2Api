...
// Use dynamic import for fs to avoid issues in edge environments
let fs: typeof import('fs') | null = null;
if (typeof window === 'undefined') {
  try {
    fs = require('fs');
  } catch {
    // fs not available in edge environment
  }
}

// Check if running in Cloudflare Workers environment
const isCloudflareWorkers = () => {
  return process.env.DEPLOYMENT_PLATFORM === 'cloudflare' || 
...
export function createCmsModule(postType: PostType) {
  ...
  /**
   * Get a single post by slug from pre-built data (Cloudflare Workers)
   */
  async function getBySlugFromBuildData(
    slug: string,
    locale: string
    ...
    try {
      ...
      // blog-data-loader doesn't exist, fall back to fs
    }
    return null;
  }
  ...
  async function getBySlug(
    ...
    // Try local files first if localDirectory is configured
    if (localDirectory) {
      // In Cloudflare Workers, use pre-built data
      if (isCloudflareWorkers()) {
        const localPost = await getBySlugFromBuildData(slug, locale);
        if (localPost) {
          return {
            post: localPost,
            error: undefined,
        ...
        // In Node.js environment, use filesystem
  ...
  /**
   * Get all posts from pre-built data (Cloudflare Workers)
   */
  async function getLocalListFromBuildData(locale: string): Promise<PostBase[]> {
    try {
      // Dynamic import to avoid loading in non-CF environments during build
      ...
      // blog-data-loader doesn't exist, return empty
      return [];
    }
  }
  ...
  async function getLocalList(locale: string = DEFAULT_LOCALE): Promise<GetListResult> {
    ...
    // In Cloudflare Workers, use pre-built data
    if (isCloudflareWorkers()) {
      const posts = await getLocalListFromBuildData(locale);
      return { posts };
    }
  ...
  /**
   * Get post metadata from pre-built data (Cloudflare Workers)
   */
  async function getPostMetadataFromBuildData(
    slug: string,
    locale: string
    ...
    try {
      ...
      // blog-data-loader doesn't exist
    }
    return null;
  }
  ...
  async function getPostMetadata(
    ...
    if (localDirectory) {
      // In Cloudflare Workers, use pre-built data
      if (isCloudflareWorkers()) {
        const metadata = await getPostMetadataFromBuildData(slug, locale);
        if (metadata) {
          return { metadata };
        }
      } else if (fs) {
